...
const Advisor = () => {
    ...
    // PDF Export Handler - Server-side with fallback to html2pdf
    const handleDownloadPDF = async () => {
        ...
        const pdfData = {
            // User info
            userName: data.personalInfo.namaLengkap || 'Pengguna',
            generatedAt: new Date().toISOString(),

            // Health metrics
            healthScore: activeSession.summary?.score || 0,
            healthGrade: diagnosis.overallHealthGrade,
            // Gauge needle coordinates (arc from -90 to 90 degrees, center at 100,100, radius 70)
            needleX: Math.round(100 + 70 * Math.cos((((activeSession.summary?.score || 0) / 100) * 180 - 180) * Math.PI / 180)),
            needleY: Math.round(100 + 70 * Math.sin((((activeSession.summary?.score || 0) / 100) * 180 - 180) * Math.PI / 180)),
            ...
            // Ratios
            savingsRatio: surplus > 0 ? (surplus / monthlyIncome) * 100 : 0,
            expenseRatio: (calculateMonthlyExpenses(data) / monthlyIncome) * 100,

            // Emergency fund
            emergencyFundCurrent: data.emergencyFund.danaDaruratSaatIni,
            emergencyFundNeeded: calculateEmergencyFundNeeded(data),
            emergencyFundPercent: data.emergencyFund.danaDaruratSaatIni / calculateEmergencyFundNeeded(data) * 100,
            emergencyFundGap: Math.max(0, calculateEmergencyFundNeeded(data) - data.emergencyFund.danaDaruratSaatIni),
            emergencyFundMonths: data.personalInfo.jumlahTanggungan >= 3 ? 12 :
                data.personalInfo.jumlahTanggungan >= 2 ? 9 :
                    (data.personalInfo.statusPernikahan === 'menikah' || data.personalInfo.jumlahTanggungan >= 1) ? 6 : 3,
            ...
            // NEW: Additional fields for new template design
            totalPages: 7,

            // Debt and Solvency Ratios
            debtRatio: monthlyIncome > 0 ? (data.debts.reduce((sum: number, d: { cicilanPerBulan?: number }) => sum + (d.cicilanPerBulan || 0), 0) / monthlyIncome) * 100 : 0,
            solvencyRatio: calculateTotalAssets(data) > 0 ? ((calculateTotalAssets(data) - calculateTotalLiabilities(data)) / calculateTotalAssets(data)) * 100 : 0,

            // Recommended Allocation - use the same pre-calculated value as the website (line 120)
            recommendedAllocation,

            // Financial Goals from user data
            financialGoals: (data as { goals?: Array<{ nama: string; targetDana?: number; jumlahTerkumpul?: number; targetWaktuBulan?: number }> }).goals?.map((goal) => ({
                name: goal.nama || 'Tujuan Keuangan',
                targetAmount: goal.targetDana || 0,
                currentAmount: goal.jumlahTerkumpul || 0,
                progressPercent: goal.targetDana ? ((goal.jumlahTerkumpul || 0) / goal.targetDana) * 100 : 0,
                timeframe: goal.targetWaktuBulan ? `${goal.targetWaktuBulan} bulan` : '12 bulan',
                priority: 'Moderat',
                monthlyAllocation: surplus > 0 ? Math.round(surplus * 0.3) : 0,
                recommendation: 'Alokasi dari surplus bulanan'
            })) || [],

            // Investment Allocation based on risk profile
            investmentAllocation: {
                deposito: data.personalInfo.usia < 30 ? 40 : data.personalInfo.usia < 40 ? 50 : 60,
                reksadana: data.personalInfo.usia < 30 ? 30 : data.personalInfo.usia < 40 ? 30 : 25,
                saham: data.personalInfo.usia < 30 ? 20 : data.personalInfo.usia < 40 ? 15 : 10,
                obligasi: data.personalInfo.usia < 30 ? 10 : data.personalInfo.usia < 40 ? 5 : 5
            }
        };

        try {
            // Try server-side HD PDF generation first
            ...
            if (response.ok) {
                ...
                a.download = response.headers.get('Content-Disposition')?.split('filename="')[1]?.replace('"', '')
                    || activeSession.pdfFileName;
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
                document.body.removeChild(a);

                console.log('[PDF] HD PDF generated via server');
                setIsGeneratingPDF(false);
                return;
            }

            // Server returned error, fall through to fallback
            console.log('[PDF] Server-side failed, using fallback. Status:', response.status);

        } catch (error) {
            // Network error or server unavailable, fall through to fallback
            console.log('[PDF] Server unavailable, using fallback:', error);
        }
        ...
        const opt = {
            margin: [10, 8, 15, 8] as [number, number, number, number],
        ...
        try {
            ...
            console.log('[PDF] Fallback PDF generated via html2pdf');
        } catch (error) {
